...
def getLLMResponse(query,age_option,tasktype_option, numberOfWords):
    ...
    prefix = """You are a {template_ageoption}, and {template_tasktype_option} within {template_numberOfWords} words: 
    ...
    print(new_prompt_template.format(template_userInput=query,template_ageoption=age_option,template_tasktype_option=tasktype_option,template_numberOfWords=numberOfWords))
    response=llm(new_prompt_template.format(
        template_userInput=query,
        template_ageoption=age_option,
        template_tasktype_option=tasktype_option,
        template_numberOfWords=numberOfWords
        ))
    print(response)
    return response
...
numberOfWords= str(st.slider('Words limit', 1, 200, 25)) #By default the value will be received in INT we need the input in STR.
